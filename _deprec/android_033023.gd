...
enum states {idle,
			...
			attack,
			airborne,
			dead} ## State that player is in to control flow of contextual action and conditions.
...
func _apply_jumping():
	...
	if !is_on_floor() and !is_on_wall()  && AIR_JUMP_COUNT > 0:
	...
	# Wall Jumping
	if is_on_wall_only() and velocity.y < WALL_JUMP_TOLLERANCE && WALL_JUMP_COUNT > 0:
