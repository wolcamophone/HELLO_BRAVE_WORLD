...
func _unhandled_input(event):
	if !paused && Input.is_action_just_pressed("Pause") && Input.mouse_mode == Input.MOUSE_MODE_CAPTURED: 
		pause_game()
	elif paused && Input.is_action_just_pressed("Pause") or event.is_action_pressed("ui_cancel"):
		unpause_game()
...
func _on_start_pressed():
	...
	GameMaster.load_level("hub_world", 0)
	unpause_game()
